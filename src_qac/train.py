...
agent = Agent(state_shape[0], plot=ax)
...
while True:
    ...
    if num_steps % 1000 == 0:
        ...
        try:
            agent.critic_model.save(path.join(model_dir, "critic_model_" + str(num_steps)))
            print(".")
            agent.actor_model.save(path.join(model_dir, "actor_model_" + str(num_steps)))
